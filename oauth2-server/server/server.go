...
// CheckAdminType
// Default false, true 일 경우 true
func (s *Server) CheckAdminType(r *http.Request) (bool, error) {
	log.Println(r.FormValue("IsCreate"))

	checkadmin, err := strconv.ParseBool(r.FormValue("IsCreate")) // false 로 잡았는데
	if err != nil {
		return false, err
	}
	return checkadmin, nil
}
