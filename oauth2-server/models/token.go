...
func (t *Token) GetAccessAPI(IsCreate bool) []string {

	db, err := sql.Open("mysql", "root:root@tcp(mysql-oauth2)/OAuth2")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	query = fmt.Sprintf("%s%s%s", "select IsAdmin FROM role where id = '", t.UserID, "'")
	err = db.QueryRow(query).Scan(&IsAdmin)
	if err != nil {
		log.Fatal(err)
	}
	...
	if IsAdmin { // admin true
		query = "SELECT scope FROM function"
	} else {
		if IsCreate { // delete & update
			query = fmt.Sprintf("%s%s%s", "SELECT scope FROM function where IsCreate = true and id ='", t.UserID, "'")

		} else { // call
			query = fmt.Sprintf("%s%s%s", "SELECT scope FROM function where id ='", t.UserID, "'")
		}
	}
