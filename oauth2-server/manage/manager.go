...
func (m *Manager) GenerateAccessToken(ctx context.Context, gt oauth2.GrantType, tgr *oauth2.TokenGenerateRequest) (oauth2.TokenInfo, error) {
	...
	// token check
	tk, err := m.tokenStore.GetByAccess(ctx, tgr.ClientID+"-"+tgr.UserID)
	if err != nil && tk.GetAccessExpiresIn() > 0 {
		return tk, nil

	}

	if tk != nil {
		if tk.GetAccessExpiresIn().Seconds() > 0 {
			return tk, nil
		}
	}

	err = m.tokenStore.RemoveByAccess(ctx, tgr.ClientID+"-"+tgr.UserID)
	if err != nil {
		return nil, err
	}
