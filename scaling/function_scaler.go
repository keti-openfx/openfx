...
// ScalingConfig
func NewFunctionScaler(config ScalingConfig, functionCacher FunctionCacher, clientset *kubernetes.Clientset) FunctionScaler {
...
func (f *FunctionScaler) Scale(functionName, namespace string) FunctionScaleResult {
	...
	queryResponse, err := GetReplicas(functionName, namespace, f.Clientset)
	...
	if queryResponse.AvailableReplicas == 0 {
		...
		scaleResult := backoff(func(attempt int) error {
			...
			setScaleErr := SetReplicas(functionName, namespace, int(minReplicas), f.Clientset)
			if setScaleErr != nil {
