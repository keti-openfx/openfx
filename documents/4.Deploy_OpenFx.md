...
- **openfx/yaml**폴더의 **docker-registry-secret.yaml**에 **.dockerconfigjson**내용을 이전에 확인한 **.dockerconfigjson**으로 변경한다
...
**openfx/yaml** 폴더의 **docker-registry-secret.yaml** 파일의 **.dockerconfigjson**의 내용을 위에서 확인한 **Secret** `regcred`의 **.dockerconfigjson** 내용으로 변경한다

# configure gateway-dep.yml

**openfx/yaml** 폴더의 **gateway-dep.yml**파일의 **image**란의 레지스트리 IP와 Port를 변경한다
...
아래 명령어를 이용하여 **OpenFx**를 배포한다
    ...
    $ cd $GOPATH/scr/github.com/keti-openfx/openfx
...
`openfx-cli`를 이용해 `echo`함수를 배포하여 `openfx`의 작동을 테스트한다
...
- 생성된 이미지를 통해 Kubernetes에 함수 배포
  ...
  ```bash
    ...
    Deploying: echo ...
    http trigger url: http://10.0.2.15:31113/function/echo
  ```
